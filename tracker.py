...
class tracker():
    def __init__(self, Mywindow_width, Mywindow_height, Mymargin, My_ym=1, My_xm=1, Mysmooth_factor=15):
        #list that stors all the past (left,right) certer set values used for smoothing the output
        self.recent_centers=[]
    ...
    #the main tracking function for finding and storing lane segment position using CONVOLUTION
    def find_window_centroids (self, warped):
        ...
        l_sum = np.sum(warped[int(3*warped.shape[0]/4):,:int(warped.shape[1]/2)], axis=0) #histogram
        l_center = np.argmax(np.convolve(window,l_sum))-window_width/2 #finding the peak
        ...
        #return averaged values of the line cernters, helps to keep the markers from jumping around too much
